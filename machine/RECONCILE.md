...
Candidate preparation acquires the pinned [tomlkit 0.15.1](https://pypi.org/project/tomlkit/0.15.1/)
into that run's staging directory through the host's existing Python `pip`, or its
existing `uv` when `pip` is unavailable. It never installs a global dependency.
