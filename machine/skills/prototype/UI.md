...
### 6. Capture the verdict and implementation handoff

Once a variant has won, record which variant won, why, and the concrete changes an
implementation task should make. Keep the variants as the planning artifact when
their retention is authorized. When the existing request authorizes implementation, apply
the selected design through the repository's normal workflow without asking again:

- **Sub-shape A**: implement the selected design on the existing page.
- **Sub-shape B**: implement the selected design as a real route.

A planning-only request ends with the artifact, verdict, and implementation
handoff. Selection alone does not authorize production changes.
...
- **Promoting the prototype directly to production.** The variant code was written under prototype constraints (no tests, minimal error handling). When implementation is authorized, rewrite the selected design under the repository's normal workflow.
